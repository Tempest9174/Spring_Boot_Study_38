...
import raisetech.student.management2.repository.StudentRepository;
  ...
  */
  ...
  public class StudentService {
    ...
    /**
     * 受講生詳細検索 IDに紐づく受講生情報を取得したあと、その受講生に紐づく受講生コース情報を取得し設定
     *
     * @param id 受講生ID
     * @return 受講生
     */
    ...
    /**
     * 受講生コース検索を行う 全件検索を行うため条件指定は行わない
     *
     * @return 受講生コース一覧
     */
    ...
    /**
     * 受講生詳細の登録を行う 受講生とコース情報を個別に登録し、受講生コース情報には受講生情報を紐づける値や日付を設定（コース開始日）
     *
...
//  }
    ...
    public void registerStudentCourse(StudentsCourse studentsCourse) {
      ...
      List<StudentsCourse> existingCourses = repository.searchStudentCourse(
          studentsCourse.getStudentId());
      System.out.println("既存コース: " + existingCourses);

      boolean duplicated = existingCourses.stream()
          .anyMatch(course -> course.getCourseName().trim()
              .equalsIgnoreCase(studentsCourse.getCourseName().trim()));
    ...
    /**
     * 受講生詳細の更新を行う 受講生情報と受講生コース情報を個別に更新する
     *
     * @param studentDetail 受講生詳細
     */
    @Transactional
    ...
    public List<StudentsCourse> searchStudentCourseList(String studentId) {
      if (studentId == null || studentId.isBlank()) {
        throw new BadRequestException("studentIdは必須です"); // 400
      }

      List<StudentsCourse> courses = repository.searchStudentCourse(studentId);

      if (courses == null || courses.isEmpty()) {
        // 警告にしたい場合は 404 エラーを投げる
        throw new StudentNotFoundException("受講生ID " + studentId + " のコースは存在しません");
      }
